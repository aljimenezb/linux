			 ...
			 unsigned int num_contig_lg2)
{
	...
	if (!(dirty->flags & IOMMU_DIRTY_NO_CLEAR)) {
		...
		/* FIXME update the gather
		if (dirty->dirty->gather)
		*/
	}
}
...
static void __replace_cut_entry(const struct pt_state *parent_pts,
				pt_oaddr_t oa, struct pt_write_attrs *attrs,
				...
				unsigned int end_index)
{
	...
	for_each_pt_level_item(&pts) {
		unsigned int pgsize_lg2 = pt_compute_best_pgsize(&pts, oa);

		if (PT_WARN_ON(pts.type != PT_ENTRY_OA) ||
		    PT_WARN_ON(!pgsize_lg2))
			continue;

		pt_install_leaf_entry(&pts, oa, pgsize_lg2, attrs);
		pts.type = PT_ENTRY_OA;
			      ...
			      const struct pt_write_attrs *parent_attrs)
{
	...
	struct pt_write_attrs attrs = *parent_attrs;

	pt_attr_from_entry(parent_pts, &attrs);

	if (!log2_mod(parent_pts->range->va, pt_table_item_lg2sz(parent_pts))) {
		/* The cut start at an item boundary */
		__replace_cut_entry(parent_pts, oa, &attrs, start_index,
				    cut_index);
		__replace_cut_entry(parent_pts, oa, &attrs, cut_index,
				    last_index + 1);
	} else {
		...
		if (cut_index != last_index)
			__replace_cut_entry(parent_pts, oa, &attrs,
					    cut_index + 1, last_index + 1);
	}
}
			      ...
			      struct iommu_iotlb_gather *iotlb_gather)
{
	...
	/* FIXME into gather, rcu */
	pt_radix_free_list(&unmap.free_list);
